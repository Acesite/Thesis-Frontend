...
function makeSerialLabel(crop) {
  const name = String(crop?.crop_name || "CROP").toUpperCase();

  // Use crop.id as the serial base, pad to 4 digits
  const rawId = crop?.id ?? "";
  const idNum = String(rawId).replace(/\D/g, ""); // keep digits only
  const padded = (idNum || "0").padStart(4, "0");

  return `${name} SN-${padded}`;
}
...
const AdminManageCrop = () => {
  ...
  const handleEdit = (crop) => {
    ...
    setEditForm({
      ...
      est_farmgate_value_display: crop.est_farmgate_value_display || "",
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {isLoading ? (
              ...
              pageItems.map((crop) => {
                ...
                // ✅ Prefer exact text from DB if present
                const estFarmgateDisplay =
                  typeof crop.est_farmgate_value_display === "string" &&
                  crop.est_farmgate_value_display.trim() !== ""
                    ? crop.est_farmgate_value_display.trim()
                    : null;
              ...
              <section className="rounded-xl border border-slate-200 bg-slate-50/60 p-4 md:p-5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  ...
                  <div className="lg:col-span-1">
                    ...
                    <input
                      name="est_farmgate_value_display"
                      value={editForm.est_farmgate_value_display || ""}
                      onChange={handleEditChange}
                      placeholder="e.g., ₱12,000 – ₱18,000"
                      className="w-full h-10 md:h-11 border border-slate-300 rounded-lg px-3 focus:outline-none focus:ring-2 focus:ring-emerald-600 bg-white"
                    />
                    <p className="mt-1 text-[11px] text-slate-500">
                      You can enter a single value (₱15000) or a range (₱12000 –
                      ₱18000).
                    </p>
                  </div>
