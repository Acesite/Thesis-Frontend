...
const SuperManageCrop = () => {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white font-poppins">
      ...
      {/* Floating Sidebar aligned to logo */}
      <aside className="fixed top-[90px] left-[350px] w-[200px] p-6 z-50">
        <nav className="space-y-4  font-medium ">
          <a href="/SuperAdminLandingPage" className="flex items-center gap-2 hover:text-green-600">
            Sugar Cane
          </a>
          <a href="/ManageAccount" className="flex items-center gap-2 hover:text-green-600">
             Rice
          </a>
          <a href="/SuperAdminManageCrop" className="flex items-center gap-2 text-green-700 font-bold">
             Corn
          </a>
        </nav>
      </aside>
        ...
        <div className="flex flex-col gap-6">
          {crops.length > 0 ? (
            ...
            <div key={crop.id} className="w-full max-w-3xl relative mx-auto mr-[560px]">
                ...
                {/* Crop Info */}
                <h3 className="text-lg font-bold text-green-700 mb-2">{crop.crop}</h3>
                ...
                <div className="mt-2 text-xs text-gray-500">
                  Tagged by: {crop.first_name && crop.last_name ? `${crop.first_name} ${crop.last_name}` : "N/A"}
                </div>
              </div>
            ))
          ) : (
